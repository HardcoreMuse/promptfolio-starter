...
export default async function UserProfilePage({ params }: Params) {
  ...
  if (pErr || !profile) {
    return <main className="p-6"><h1 className="text-xl">User not found</h1></main>;
  }
        ...
        <ul className="divide-y border rounded">
          {(cards ?? []).map(c => (
            <li key={c.id} className="p-3">
              ...
              {c.model_hint && <span className="ml-2 text-xs text-gray-500">({c.model_hint})</span>}
            </li>
          ))}
          {(!cards || cards.length === 0) && <li className="p-3 text-gray-500">No public prompts yet.</li>}
        </ul>
      </section>
    </main>
  );
